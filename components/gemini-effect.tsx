...
export const GeminiEffect = ({
  ...
  // Set up and update the path animations based on pathValues
  useEffect(() => {
    if (!svgRef.current) return;
    
    const paths = svgRef.current.querySelectorAll<SVGPathElement>('path.animated-path');
    
    paths.forEach((path, index) => {
      try {
        const totalLength = path.getTotalLength();
        
        // Initialize path styling once
        if (!path.style.strokeDasharray) {
          path.style.strokeDasharray = `${totalLength}`;
          path.style.strokeLinecap = "round"; // Rounded ends for smoother appearance
        }
        
        // Use GSAP for smoother transitions of the offset
        if (index < pathValues.length) {
          const offset = (1 - pathValues[index]) * totalLength;
          gsap.to(path, {
            strokeDashoffset: offset,
            duration: 0.1, // Quick but smooth transition
            ease: "power1.out",
  ...
  return (
    <div className={cn("absolute inset-0 flex items-center justify-center", className)}>
      ...
      >
        ...
        {/* Background paths with blur effect - these create the glow */}
